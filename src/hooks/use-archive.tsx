...
export function ArchiveProvider({ children }: { children: ReactNode }) {
  ...
  const handleNewArchiveData = (archiveData: any, sourceUrl?: string) => {
    ...
    if (existingArchiveIndex > -1) {
      ...
      updatedArchives[existingArchiveIndex] = { 
        ...newArchive, 
        isOnline: existingArchive.isOnline, 
        sourceUrl: existingArchive.sourceUrl 
      };
      if (sourceUrl) { 
  ...
  const forkArchive = (id: string) => {
      ...
      const forkedArchive: Archive = {
          ...JSON.parse(JSON.stringify(originalArchive)),
          id: newId,
  ...
  const restorePost = (post: Post, archiveId: string) => {
    const updatedArchives = archives.map((archive) => {
      if (archive.id === archiveId) {
        if (archive.posts.some(p => p.id === post.id)) return archive;
        const newPosts = [...archive.posts, post].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        return { ...archive, posts: newPosts };
      }
      return archive;
    });
  ...
  const deletePost = (id: string) => {
    if (!activeArchiveId || activeArchive?.isOnline) {
        toast({ title: "Action not allowed", description: "Cannot delete posts from an online archive.", variant: "destructive" });
        return;
    }
    const postToDelete = activeArchive?.posts.find(p => p.id === id);
    if (!postToDelete) return;

    const archiveId = activeArchiveId;

    const updatedArchives = archives.map((archive) =>
        archive.id === archiveId
            ? { ...archive, posts: archive.posts.filter((p) => p.id !== id) }
            : archive
    );
    saveArchives(updatedArchives, archiveId);

    toast({
        title: "Post Deleted",
        description: `"${postToDelete.title}" has been deleted.`,
        action: <Button variant="outline" onClick={() => restorePost(postToDelete, archiveId)}>Undo</Button>,
    });
  };
